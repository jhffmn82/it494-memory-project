> **PHASE 1 IS COMPLETE and this document is historical.** It is kept because two things in it
> are still binding and are recorded nowhere else: the three-bucket provenance rule and the style
> standard, both below. The deliverable table describes a document layout that no longer exists;
> the current index is `docs/README.md`.
>
> Path corrections applied 2026-08-28: deliverables 3, 6, 7 and 8 were numbered by ask order while
> the files were numbered by build order, so four rows pointed at filenames that never existed.
> They now point at the files' current locations.
...
- Anatomy first: docs/reference/07_pipeline_anatomy.md (in progress) inventories every by-hand stage of the mock with code receipts; its by-hand table is the harness component backlog.

## Deliverables, in build order

| # | Ask | Output | Depends on |
|---|-----|--------|------------|
| 0 | Context mining: his records searched for goals, time, constraints he did not restate | `docs/reference/00_design_brief.md` (internal, shapes everything below) | running |
...
| 3 | Reference digest: one paragraph per work, organized by work | `docs/reference/03_digest.md` | 2 |
| 4 | Topic narrative: major topics, approaches traced by work and date | `docs/reference/01_topic_narrative.md` | 2; topic set shown to Justin before final |
| 5 | His proposal as it stood before the research, in his voice, from his record only | `docs/reference/02_proposal_draft.md` (DRAFT until he corrects it) | 0, sealed from 1-4 |
| 6 | Elevation options: itemized approaches from the research, each traced to source and checked against what the system already does | `docs/reference/04_elevation_options.md` | 2, 0 |
| 7 | Three project shapes, with real calls made, against the actual calendar | `docs/archive/05_project_shapes.md` | 0, 6 |
| 8 | End product outline: what to build, test data options, what to record, QA checkpoints for his line-by-line review | `docs/archive/06_end_product_and_testing.md` | 7 |
| 9 | Master PDF, beautifully formatted, with index | `build/IT494_Phase1_Research_Package.pdf` | all |
