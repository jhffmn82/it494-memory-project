...
ns = load(ROOT / "notebooks" / "threadatlas-ingestor.py", SCR)
...
# valid_to is dropped (09-09): the model is not asked for it and no fact carries it
check("no fact carries valid_to, and the fact schema does not ask for one (09-09)",
      "valid_to" not in json.dumps(ns["FACT_SCHEMA"]) and by["fact"] and all("valid_to" not in f for f in by["fact"]),
      len(by["fact"]))

# an empty chat turn (55 in the 1.7 export) is left out and costs no call
spoken = "SESSION s TURN 1\nuser: where is the market?\n\n"
silent = "SESSION s TURN 2\nassistant: \N{ZERO WIDTH SPACE}\n\n"
turns = {"text": spoken + silent, "units": [
    {"position": 0, "start": 0, "end": len(spoken), "kind": "user", "label": "SESSION s TURN 1"},
    {"position": 1, "start": len(spoken), "end": len(spoken + silent), "kind": "assistant", "label": "SESSION s TURN 2"}]}
book = {"text": "CHAPTER I", "units": [{"position": 0, "start": 0, "end": 9, "kind": "body", "label": "I"}]}
check("an empty chat turn is left out; a spoken turn and a book's unit are not (1.7)",
      ns["empty_turn"](turns, turns["units"][1]) and not ns["empty_turn"](turns, turns["units"][0])
      and not ns["empty_turn"](book, book["units"][0]))
check("a chat of one spoken turn and one empty one is one reading", ns["one_reading"](turns))
