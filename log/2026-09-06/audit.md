...
31. **Back to bottom-up at the end, and a 0.6 that is smaller** (Justin's ruling, 09-07, after the 0.5 run on Kaggle: the per-unit judge had spent $0.53 by chapter XIV against $0.14 at the same point in 0.4, sixty-odd "different" verdicts a chapter, and the run stalled and ended). Every unit is read on its own with no look back: no roster, no `continues`, no previous summary. All merging happens at the end: the unit-locals go into an agglomerative priority queue of pairs (a unit-major in every pair, never minor against minor; the nomination's tier first, then the name, co-occurrence and profile scores), the judge takes ten a call from the top, unsure waits for one last look; then the facts of each major are consolidated; then predicates are merged pairwise, similarity nominating the pairs and the judge saying whether one name fits both and what it is. An instant match still needs both sides named (28's second half kept: the same proper name and kind unites without a judge). A quote with an ellipsis is located piece by piece and kept, since "Quelala ... the best and wisest man" was a real fact dropped as a paraphrase. Rulings 28's rolling mechanism and 31's document-level checkpointing are withdrawn; the sidecar keeps the units only. The file went from 3,000 lines to 2,284; the chat's own "words" matcher (a near-verbatim citation located by its words) was built and taken out again as unruled.

32. **Predicates are not merged, and not renamed** (Justin, 09-07, correcting 31's "predicates merged pairwise" within the hour, and repeating his ruling of 09-06). Facts are searched semantically; a merged or renamed predicate fits the original less well and is one more place to be wrong. The pairwise judge is out; the adjudication names consolidated facts as the listed facts name them. The chat had brought merging back twice against his ruling and records that here. Also that morning: a citation reworded at the edges is located by its words and stored as the text's own passage (`words`, decision 36); the adjudication sets aside a fact whose passage does not state it, ranked `unsupported`, at no extra call (37); quotes are stored as text beside their offsets (38).
