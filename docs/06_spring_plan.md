...
**2026-08-27.** A separate project from `05_fall_plan.md`, with its own deliverable, its own risks and
its own administrative gates. The fall produces a measured backend; the spring turns it into
...
police. This is what closes the multi-tenancy gap from the archived schema quality pass (`archive/15_schema_and_architecture.md`) for this
deployment.
