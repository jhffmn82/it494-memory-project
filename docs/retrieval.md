...
parent summary also has rows (`record` = `parent`, no `doc_id`); a parent is a route, never a
record the reader sees. The unit of packing is the record, never a sentence.
...
score(r) = max over the rows i of record r of s[i]
V        = the K_ENTRY records with the largest score
```
...
W = select record, doc_id, record_id from search where search match q_or [and doc_id in filter]
    order by bm25(search) limit K_ENTRY
```
...
```
RRF(r) = sum over arms a listing r of  1 / (RRF_K + rank_a(r))
E      = the records of V and W ordered by RRF, descending
```

A record in both lists scores about twice one in either. With one arm off, E is the other's list.
...
A parent summary that is an entry hit has one edge, to its children's records inside the
filter; the summary itself never enters the pool. Entry records have `hops = 0`; a pulled record
has `hops = 1` and keeps the first route that reached it.
...
A direct hit outranks what it pulled in; a pulled record scores on its own content, not its
route.
...
false); packed and misread by the reader. The context rebuilds byte for byte from the log.
