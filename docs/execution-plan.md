...
land. Ruled on the audit's point 2: the fall scope is ingest, the global layer, storage, retrieval, the test harness, and test and assessment, plus wiki pages over document clusters (an afternoon's work, after the numbers, rendered from the store the way `render_package.py` renders a package); then publish. Maintenance, deployment and a living stream of data are spring. Corrected on the audit's point 3, from the 09-08 review and decision record: co-occurrence is the global layer's second signal (the up-edge decision takes the child's name and the cast it co-occurs with, scored and logged separately per candidate parent), and the document clusters behind the wiki pages come from the same document-entity graph (related works from shared entities). The resolution ablation is therefore a fall measurement, restated at the up-edge: name-only versus name plus co-occurrence as inputs to attachment, replayed from the logged scores, scored as attachment accuracy against the hand-labeled Oz alias set (one hour of labels, written in week 2) and, on LongMemEval, as the retrieval difference between the two attachments. Cut from the fall: NarrativeQA and the cells ablation.
...
| 4 | Oct 26 to Nov 1 | none; 2g's tables only | the paper complete in ECIR format (12 pages plus references, Springer LNCS, single-blind); the dataset DOI minted so the paper can cite it | **Oct 30**: the paper complete |
| 5 | Nov 2 to 8 | none | **Nov 2: submitted to ECIR 2027 resource track**; Nov 3: the arXiv preprint (endorser lined up); the same week: Dr. Fang gets the submitted paper for the letter | submitted |
| 6 | Nov 9 to Dec 15 | none | the 8-page ACL-format version prepared against the EACL workshop list (out Oct 2); **Dec 7**: ECIR decision; if rejected, **Dec 15**: EACL workshop submission, and PVLDB's Jan 1 rolling deadline as the third | a decision or a submission in hand before applications |
...
- The venue (added on the audit's goal 9): the paper has to be submitted to a peer-reviewed
  ...
  deadlines between November and January. Checked on 09-13 (`log/2026-09-13/venues.md`). Ruled 09-13: ECIR 2027 resource track, deadline Nov 2 (notification Dec 7, before every application deadline; single-blind; arXiv allowed; 12 pages), which pulls the freeze to about Oct 25; if it is rejected, an EACL 2027 workshop by Dec 15 (list out Oct 2; notification Jan 5) or PVLDB's Jan 1 rolling deadline. The arXiv preprint needs a personal endorser (Dr. Fang) under the January 2026 policy, since automatic endorsement requires a prior arXiv paper; that is the endorsement email's real content.
