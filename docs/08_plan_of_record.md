...
6. Turn on consult-logging in the mock this week: each session records what memory was cued, what was read, and what the answer used, so the miss-rate clock starts accruing from September. This is mock-side instrumentation, exempt from the authorship rule, and no later phase builds it otherwise.
...
The fall commitment, after adversarial feasibility review, is the core: the instruments running on the mock, one two-arm comparison on the archive's corrected-facts band, the synthesis memo, the design document with the segmentation spec, and the paper frozen by November 15. The pipeline build below proceeds in order as far as the block allows, ingest and organize first, and everything past the commitment is stretch, not promise: the review priced this phase at two to three times its boxes against a realistic 24 to 32 hour block, and the plan now says so rather than footnoting it.
...
Applied from the feasibility review: all fall measurement runs inside November 1 to 15, and the paper posts by November 15, because the weeks after it are dead by the design brief's own calendar. What remains in this window is submission logistics, the brag sheets, and the applications themselves.
