...
| 09-14 | The ingestor is unfrozen for one chat run (1.8): the session reading gives each entity a kind (never `thing`) and a salience mark; a minor's facts are stored on the session's document node with `direction` `mentioned` and the entity's name in `subject_name`. This reverses the 09-13 night line that left kind and salience to the global layer, and narrows the 09-11 minor-subject rule for chats. | chat; log/2026-09-14/note-to-ingestor-thread.md | refined the same day: the kind and salience come from a final call after the reading, and the run covered all 81 documents |
| 09-14 | Chat kind and salience come from one final Luna call per session after the unchanged reading, not from the reading itself; a `thing` is refused and stored null; the user stays a major person. | chat (the ingestor thread); log/2026-09-14/ingestor/receipt.json | stands |
| 09-14 | Every document node carries deterministic facts from the export (title, author, date, source class, a chat's history) with no quote and no unit, `provenance.from` `document record`; nothing searches the text for them. Marked for the future: the extractor exports the offsets of the lines it verified so these facts can carry quotes. | chat (the ingestor thread) | stands |
| 09-14 | Ingestor 1.8 ran over all 81 test documents, replacing the 09-13 packages; the Step 1 dataset's version 2 is packed from it. Not ruled, not built: chat aliases, a one-line entity description, relative dates into `valid_from`, the 2e output shape. | chat; log/2026-09-14/note-to-ingestor-thread.md | stands |
| 09-14 | The global layer is one blocked notebook script in the ingestor's form (`notebooks/threadatlas-global-layer.py`, schema and pseudocode up top, a connection test early), not a module package; the pipeline is dry-run with the model stubbed before a kernel is pushed. | chat; log/2026-09-14/global-layer.md | stands |
| 09-14 | The offer floor on the vector is loose (0.75), because the judge is the gate and the floor only bounds the bill; measured over the store before it was set. | chat; log/2026-09-14/global-layer.md | stands |
| 09-14 | The build over a corpus is bottom up, in batches: a pool of entities (document entities aside), a heap of eligible pairs best first, batches judged in parallel with each side shown as its cluster, pairs ruled same united, pairs ruled different a constraint that keeps their clusters apart, until no valid pair remains; an is_a link goes straight into the heap; the parents are written once at the end. The incremental attach of the design note is the nightly-pull path, not built this fall. | chat; docs/global-layer.md; log/2026-09-14/global-layer.md | stands |
...
| 09-15 | The query path is hybrid: BM25 over one FTS table of records and the cosine scan over the sidecar, each ranked inside the document filter, fused by reciprocal rank; one hop along node, document and parent with the top K_HOP neighbours by their own score; rerank by own score with a hop discount; whole records packed by rank; a log per question. No term extraction from the question, no model reranking, no query rewriting this fall. `log/2026-09-15/retrieval.md` with its section 11 is the manual. | chat; log/2026-09-15/retrieval.md | stands |
| 09-15 | A parent routes and is never evidence: a parent summary may be an entry hit and is replaced by its children's records; it is never packed; with the parent hop off, parent rows are masked from the entry scans too. | chat; log/2026-09-15/retrieval.md section 11 | stands |
| 09-15 | The serving store is lean: document (no text), unit, node, alias, fact (13 columns), adjudicated_fact, cell, abstract, parent, instance_of, collection, document_in, vec_row, and one FTS table `search` with a row per record; the build's evidence (pair, merge, offers) and Step 1's diagnostics (piece, edge, ledger, candidate, rejection, completion, attribute, contradiction, provenance, tier) go to a build log beside it. The document text stays in Step 0 and is used at load to prove every quote. | chat | stands |
| 09-15 | The wiki: the static radial figure leads; the slate scheme (white paper, a pale blue-grey band, a light tint on the rules); a breadcrumb up to the collection under every entity and document title; a portal lists every entity its documents hold by relevance; a document is the parent of its mentions, decided in a final pass after the clustering; collections merge on the overlap of the smaller group. | chat; log/2026-09-15/global-layer.md | stands |
| 09-15 | Retrieval is its own pipeline, not a block of the global-layer notebook; it imports the store's dataset (`threadatlas.sqlite` and `threadatlas.npy` together) and nothing else of the build. The global layer ends at the store, the build log and the wiki. | chat; log/2026-09-15/global-layer.md | stands |
| 09-15 | Retrieval, four edge cases (GPT's review, brought by Justin): an entry is a record or a parent route, the pool holds records only; the hop discount biases toward direct hits and does not guarantee them; every order is total (score, then key ascending; RRF ranks 1-based); an abstract entry has no document edge, a parent entry the parent edge only. `docs/retrieval.md` approved with these. | chat; docs/retrieval.md | stands |
| 09-15 | Retrieval's expansion is a typed traversal over the store's representations, the same from every entry, no branch on the question or the corpus: route (fact to cell and abstract; child to its parent's other children's abstracts; parent entry to its children's abstracts), traverse (a cell to the same entity's previous and next cell; an abstract to its cells in order), substantiate (a cell to the entity's facts in that unit; an abstract to its facts; a fact carries its quote). The unit-neighbourhood edge is gone. Facts stay entry hits. The packing unit is a bundle: a cell with its facts and quotes; an abstract alone; a fact alone when its cell is absent. Sections 4 to 7 of docs/retrieval.md. | chat, after GPT's second review; docs/retrieval.md | stands |
...
| 09-14 | Anchors-first attachment (settle exact-name-plus-kind children corpus-wide, then the ambiguous ones; outside feedback of 09-14) is declined: chat entities arrive as kind `thing` (1,001 of 1,001 in the test packages), so name-plus-kind has no signal where it matters, and kind is assigned by the global layer itself (ruling 3 of 09-13). Nomination stays a similarity search (ruling 11); children are attached in document order; order sensitivity is measured by the insertion check, not designed away. | chat, on the global-layer question B | stands |
