...
| 09-13 | The global layer is built regardless of which benchmark needs it. The order is the global layer, the store, the embedding sidecar and the query path first, because they lock the design; then chat digestion in Kaggle batches (about 36 kernel hours for all chats), the smaller corpora first; testing by the end of September, October for refinement and building the datasets. | chat; docs/execution-plan.md section 1 | stands |
