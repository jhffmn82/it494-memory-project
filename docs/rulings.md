...
| 09-13 | The global layer is built regardless of which benchmark needs it, and first: the database schema and the embedding follow from it; then the query path; then a harness that tests the query path; only when each has a solution does the full data run, in Kaggle batches (about 36 kernel hours for all chats on the frozen version, less now), the smaller corpora first. Until then every step works on the test packages already on disk. Testing by the end of September; October for refinement and building the datasets. | chat; docs/execution-plan.md section 1 | stands |
