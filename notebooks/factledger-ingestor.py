...
- "facts": each durable relationship or fact stated once, with "predicate" (a clear relation in lowercase_snake_case and the present tense: where a listed fact's predicate is awkward or misnamed, has_aunt for a family tie or is_carried_to for a place, name the relation properly here; the listed facts keep their own names underneath), "object", "qualifiers" (or null), and "from": the numbers of every listed fact it is drawn from. A fact drawn from nothing listed is not allowed.
