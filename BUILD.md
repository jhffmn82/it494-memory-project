...
beside it). Retrieval is its own pipeline over the store (drafted; the rule is `docs/retrieval.md`): keyword
...
report at the tiers run (one this fall; the three-tier spread is spring); the
spread between tiers is a finding, and running everything on the best model
erases it. Batch each chunk's cell calls into one
from the start; re-sending the chunk text per entity is 2.3x on total input
and 5x on the part that dominates.

## Three checks before the wiki ships (the fall's pages are over document clusters; the full checks apply in spring)
